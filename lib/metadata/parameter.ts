...
export const getParameterMetadata = (key: string | symbol, target: object): unknown[] => {
...
export const parameterLabel = (key: string, label: string) =>
  parameterMetadata('label', (prev: unknown) => {
...
export const getParameterTags = (target: object, parameterIndex: number): Set<string> => {
  const all = getParameterMetadata('tag', target);
