...
The order of pipes affects behavior. Generally, follow this pattern:

1. **Arguments first** - `args()`, `argsFn()`
2. **Transformations second** - `decorate()`
3. **Lazy third** - `lazy()`
4. **Singleton last** - `singleton()`
5. **Access control last** - `scopeAccess()`

<CodeBlock
  code={`// ✅ Good order
R.fromClass(Service)
  .pipe(
    args('config.json'),        // 1. Arguments
    lazy(),                     // 2. Lazy
    singleton(),                // 3. Singleton
    scopeAccess(adminOnly)      // 4. Access control
  )

// ❌ Bad order - singleton won't cache lazy instances properly
R.fromClass(Service)
  .pipe(
    singleton(),               // Wrong - caching happens before lazy
    lazy(),                    // Wrong - each resolve creates new proxy
    args('config.json')        // Wrong - args applied after lazy wrapper
  )`}
  lang="typescript"
/>

<h2 id="how-pipes-work" class="toc-link">How Pipes Work Internally</h2>

### IProvider.pipe() Implementation

The `IProvider.pipe()` method accepts both raw functions and ProviderPipe objects:

<CodeBlock
  code={`pipe(...mappers: (MapFn<IProvider<T>> | ProviderPipe<T>)[]): IProvider<T> {
  const fns = mappers.map((m): MapFn<IProvider<T>> =>
    isProviderPipe<T>(m) ? m.mapProvider.bind(m) : m
  );
...
When you pass a ProviderPipe, it extracts the `mapProvider` method. Raw functions are used as-is.

### IRegistration.pipe() Implementation

Similar to `IProvider.pipe()`, but stores transformations for later application:

<CodeBlock
  code={`pipe(...mappers: (MapFn<IProvider<T>> | ProviderPipe<T>)[]): this {
  const fns = mappers.map((m): MapFn<IProvider<T>> =>
    isProviderPipe<T>(m) ? m.mapProvider.bind(m) : m
  );
...
### @register() Decorator Processing

The `@register` decorator accepts both `MapFn<IRegistration>` and ProviderPipe:

<CodeBlock
  code={`export const register =
  (...mappers: Array<MapFn<IRegistration> | ProviderPipe>) =>
  setClassMetadata(METADATA_KEY, (acc: MapFn<IRegistration>[] | undefined) => {
    const result = mappers.map((m) =>
      isProviderPipe(m)
        ? (r: IRegistration) => m.mapRegistration(r)  // Use mapRegistration
        : m                                            // Use as-is
...
When processing ProviderPipes, the decorator calls `mapRegistration()` to transform the registration.

### Type Guard for ProviderPipe

The container uses a simple type guard to detect ProviderPipes:

<CodeBlock
  code={`export const isProviderPipe = <T>(obj: unknown): obj is ProviderPipe<T> =>
  obj !== null &&
  typeof obj === 'object' &&
  'mapProvider' in obj;`}
  lang="typescript"
...
Always apply pipes in this order for predictable behavior:

1. **Binding and scope** - `bindTo()`, `scope()`
2. **Arguments** - `args()`, `argsFn()`
3. **Decorators** - `decorate()`
4. **Lazy loading** - `lazy()`
5. **Singleton** - `singleton()`
6. **Access control** - `scopeAccess()`

### Use ProviderPipes for Reusability

Create ProviderPipes (not registration pipes) when you need reusability across both provider and registration contexts:

<CodeBlock
  code={`// ✅ Good - ProviderPipe, works everywhere
export const cached = <T>() => registerPipe<T>((p) =>
  new SingletonProvider(p)
);

// ❌ Limited - Only works with registrations
export const cached = <T>(): MapFn<IRegistration<T>> =>
  (r) => r.pipe((p) => new SingletonProvider(p));`}
  lang="typescript"
/>

### Prefer Built-in Pipes

Use built-in pipes when possible - they're well-tested and optimized:

<CodeBlock
  code={`// ✅ Good - use built-in singleton()
@register(bindTo('Service'), singleton())
class MyService {}

// ❌ Avoid - custom implementation
@register(bindTo('Service'), customSingleton())
class MyService {}`}
  lang="typescript"
/>
...
Each pipe should do one thing well. Avoid creating complex pipes that combine multiple concerns:

<CodeBlock
  code={`// ✅ Good - focused pipes
@register(
  bindTo('Service'),
  singleton(),
  lazy(),
  scopeAccess(adminOnly)
)

// ❌ Bad - mega-pipe doing everything
@register(
  bindTo('Service'),
  megaPipe({ singleton: true, lazy: true, admin: true })
)`}
  lang="typescript"
/>
...
Always document what your custom pipes do and any side effects:

<CodeBlock
  code={`/**
 * Logs all method calls on the resolved instance.
 *
 * @param logger - The logger instance to use
 * @returns ProviderPipe that wraps instances with logging
 *
 * @example
 * @register(bindTo('UserService'), withMethodLogging(console))
 * class UserService {
 *   getUser(id: string) { ... }
 * }
 */
export const withMethodLogging = (logger: Console) =>
  registerPipe((provider) => { ... });`}
  lang="typescript"
/>
...
<h2 id="troubleshooting" class="toc-link">Troubleshooting</h2>

### "Cannot use bindTo in Provider.pipe()"

**Problem:** Trying to use registration-only pipes with `Provider.pipe()`

<CodeBlock
  code={`// ❌ Wrong
Provider.fromClass(Service).pipe(bindTo('Service'))

// ✅ Correct
Registration.fromClass(Service).pipe(bindTo('Service'))
// or
Provider.fromClass(Service).bindToKey('Service')`}
  lang="typescript"
/>

### Singleton Not Caching Lazy Instances

**Problem:** Wrong pipe order - singleton before lazy
...
### Arguments Not Being Injected

**Problem:** Arguments applied after lazy wrapper

<CodeBlock
  code={`// ❌ Wrong order
.pipe(lazy(), args('value'))

// ✅ Correct order
.pipe(args('value'), lazy())`}
  lang="typescript"
/>

<h2 id="advanced-topics" class="toc-link">Advanced Topics</h2>

### Pipe Composition with Functional Programming

Leverage functional composition for complex transformations:

<CodeBlock
  code={`import { pipe } from 'fp-ts/function';

const createCachedLazyService = <T>() =>
  pipe(
    args<T>(),
    lazy<T>(),
    singleton<T>()
...
@register(bindTo('Service'), createCachedLazyService())
class MyService {}`}
  lang="typescript"
/>

### Type-Safe Pipe Builders

Create type-safe builders for common pipe combinations:

<CodeBlock
  code={`class PipeBuilder<T> {
  private pipes: ProviderPipe<T>[] = [];

  singleton(): this {
    this.pipes.push(singleton<T>());
    return this;
  }

  lazy(): this {
    this.pipes.push(lazy<T>());
    return this;
  }

  build(): ProviderPipe<T> {
    return registerPipe((p) =>
      this.pipes.reduce((acc, pipe) => pipe.mapProvider(acc), p)
    );
  }
}

// Usage
const myPipe = new PipeBuilder()
  .lazy()
  .singleton()
  .build();`}
  lang="typescript"
