...
**Most pipes can be applied in any order** - `singleton()`, `lazy()`, `args()`, `argsFn()`, and `scopeAccess()` are designed to work regardless of their position in the pipe chain.

**Exception:** The `decorate()` pipe order matters relative to `lazy()`. Place `decorate()` before `lazy()` to wrap the actual instance, or after to wrap the lazy proxy.
...
### Decorator Not Wrapping Actual Instance

**Problem:** `decorate()` applied after `lazy()` wraps the proxy instead of the real instance

<CodeBlock
  code={`// If you want to decorate the actual instance:
.pipe(decorate(withLogging), lazy())  // ✅ Decorates real instance

// This decorates the lazy proxy instead:
.pipe(lazy(), decorate(withLogging))  // Decorates proxy, not instance`}
  lang="typescript"
/>
