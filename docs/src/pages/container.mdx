...
title: Scoped Dependency Injection Container
description: Manage TypeScript dependency injection scopes, request lifecycles, child containers, instance tracking, lazy resolution, and disposal
---
...
The scope creation process ensures that lifecycle hooks are properly inherited, all currently matching registrations are available, and the new scope is registered in the parent's scope list for inspection and instance traversal.

<div class="callout callout-important">
  <div class="callout-title">[!IMPORTANT]</div>
  <p>Scope creation is snapshot-like. Existing parent registrations are applied when <code>createScope()</code> is called.</p>
</div>

<div class="callout callout-warning">
  <div class="callout-title">[!WARNING]</div>
  <p>Registrations added to a parent after a child scope already exists are not pushed into that child. Create a new scope or add the registration to the child explicitly.</p>
</div>
...
<h2 id="dynamic-tag-management" class="toc-link">Dynamic Tag Management</h2>
...
- **Environment-based configuration** - Add tags based on `NODE_ENV` or other runtime configuration
- **Feature flags** - Enable/disable features by adding tags conditionally
...
<div class="callout callout-warning">
  <div class="callout-title">[!WARNING]</div>
  <p>Tags must be added <strong>before</strong> registrations are applied. Scope matching happens at registration time, so adding tags after registration will not retroactively make providers available.</p>
</div>

<CodeBlock
  code={addTagsSpecCode}
  lang="typescript"
  filename="__tests__/readme/addTags.spec.ts"
/>
...
<div class="callout callout-important">
  <div class="callout-title">[!IMPORTANT]</div>
  <p>A service's resolution context is determined by <strong>where it is registered</strong>, not where it is requested from.</p>
</div>

<div class="callout callout-warning">
  <div class="callout-title">[!WARNING]</div>
  <p>Parent-scoped services cannot inject child-scoped dependencies. Use <code>scopeAccess()</code> when a parent provider should be visible only to selected invocation scopes.</p>
</div>
...
<h2 id="checking-registrations" class="toc-link">Checking Registrations</h2>
...
- Useful for conditional registration patterns and validation

<CodeBlock
  code={hasRegistrationSpecCode}
  lang="typescript"
  filename="__tests__/readme/hasRegistration.spec.ts"
/>
...
Proper resource cleanup is essential for preventing memory leaks. The container provides a disposal mechanism that cleans up the current container's local resources and prevents further usage.
...
<div class="callout callout-important">
  <div class="callout-title">[!IMPORTANT]</div>
  <p>Disposal is local to the container being disposed. This is intentional so host frameworks can own page, request, component, or widget lifecycles explicitly.</p>
</div>

<div class="callout callout-warning">
  <div class="callout-title">[!WARNING]</div>
  <p>Child scopes are not automatically disposed when a parent is disposed. Dispose each child scope explicitly when its lifecycle ends.</p>
</div>

<h2 id="lazy-loading" class="toc-link">Lazy Loading</h2>

Lazy loading is designed only for class instances. It defers class instantiation until the resolved proxy is actually accessed, which can improve startup performance and enable circular dependency resolution.

<div class="callout callout-important">
  <div class="callout-title">[!IMPORTANT]</div>
  <p><code>lazy</code> is designed only for class instances resolved from class providers.</p>
</div>
...
- Child scopes are not automatically disposed when parent is disposed
...
- **Use lazy loading for expensive class instances** - Defer SMTP clients and external API clients until actually needed
...
- `hasRegistration(key: DependencyKey): boolean` - Check if a registration exists in the current container or parent containers
- `getRegistrations(): IRegistration[]` - Get all registrations from current container and parent containers
- `dispose(): void` - Dispose the container's local resources
...
- `addTags(...tags: Tag[]): void` - Add one or more tags to the container dynamically
