...
When `setArgsFn` is set on a provider it controls the final args. Without it, resolve-time `args` are passed through by default. Use `setArgsFn(resolveByArgs)` to resolve `InjectionToken` arguments:

```typescript
// resolveByArgs resolves tokens, passes primitives as-is
...
// UserRepositoryToken is resolved and its value passed as the constructor arg
