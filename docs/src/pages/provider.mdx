...
Implement `IProvider` directly to create a fully custom provider, or compose existing behaviors on `Provider` via `.map()`, `.singleton()`, `.lazy()`, `.addArgsFn()`, and `.addAccessRule()`:
<CodeBlock
  code={`class CustomProvider implements IProvider<T> {
  resolve(container: IContainer, options: ProviderOptions): T {
    // Your custom resolution logic
  }
  // ...other IProvider members
}`}
  lang="typescript"
/>
