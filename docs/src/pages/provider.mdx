...
<ul>
  ...
  <li>
    <strong>Factory Provider</strong>: <code>new Provider((container, ...args) =&gt; ...)</code> - Custom factory function
  </li>
</ul>
...
<p>
  ...
  dependencies using <code>ScopeAccessRule</code>. This is useful for implementing access control, feature
...
### Provider Lookup
<p>
  Providers are stored in a <code>Map</code> keyed by <code>DependencyKey</code>, providing O(1) lookup performance. Alias resolution uses a separate <code>AliasMap</code> for efficient multi-key lookups.
</p>
