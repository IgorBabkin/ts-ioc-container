...
`IContainer.getInjector()` hands out the injector shared by the whole scope tree (ADR 0014 reversed ADR 0012's "no accessor" rule). All three modules are `IContainerModule`s applied with `container.useModule(...)`, each taking the runner which performs what it collects:
`new Container().useModule(new OnConstructModule(runner))`.
`OnConstructModule` reaches the injector through `getInjector()`; `OnResolvedModule` reaches providers through `onRegistered`. `@hook` has no "no hook means invoke the method" shorthand — always name the hook.

Prefer resolve hooks — `OnResolvedModule`, or the `onResolve(...)` pipe — over construct hooks when documenting or writing hooks: construction is the injector's event, so `OnConstructModule` misses dependencies the injector never builds (`fromValue` constants, factory registrations) and observes the instance before the provider's `decorate(...)` chain. A resolve hook runs on every dependency leaving a provider, after decoration.
