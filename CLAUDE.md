...
Hooks are async-capable by default, so there is no `Async`-postfixed decorator or module (ADR 0013). A decorated member carries **exactly one** hook (ADR 0015) — `@hook(key, fn)` takes a single `HookType`, and decorating one member twice under the same key replaces the earlier hook (decorators apply bottom-up, so the topmost stays). Several hooks are combined where they are declared, by higher-order functions in `lib/hooks/combinators.ts`: `sequential(...hooks)` (declaration order, awaiting each one that goes async), `parallel(...hooks)` (all started at once) and `oncePerInstance(hook)` (a single run per instance). All three return a plain `HookFn`, so they nest — `oncePerInstance(sequential(a, parallel(b, c)))` — and a custom combinator needs nothing but `toHookFn`. **Running hooks is the consumer's job (ADR 0016): the library only collects them.** `HookCollector` (`lib/hooks/HookCollector.ts`) is constructed with the hook `key` it reads (`onConstruct`, `onScopeDisposed`, `onResolved`, or a custom key), and `collector.getActions(target, { scope })` returns one `HookAction` (`{ hook, context }`, the member it came from being `context.methodName`) per decorated member, in declaration order, with hook classes resolved to functions and contexts built. Nothing has run when it returns. A runner — a function of the collected actions, typed by the application, since the library neither calls one nor takes one — performs them. **The library also ships no hook modules (ADR 0018):** `OnConstructModule`, `OnDisposeModule`, `OnResolvedModule` and `resolved(...)` are gone, and an application wires collection to the container's own events itself — `container.getInjector().onConstructed(...)`, `container.scopeDisposed.subscribe(...)`, `container.registered.subscribe(...)` plus `provider.onResolved(...)`, or the `onResolve(...)` pipe for one registration. `IContainerModule` is one method, so such a module is a function returning `{ applyTo }`; `__tests__/hooks/modules.ts` holds the three as consumer code. `toTask(action)` turns an action into the `Task` that `runInOrder` / `runAtOnce` (`lib/utils/task.ts`) take, so the strategies this replaced are one line each: `(actions) => { for (const { hook, context } of actions) hook(context); }`, `(actions) => void runInOrder(actions.map(toTask))`, `(actions) => void runAtOnce(actions.map(toTask))`. There is no `onError` anywhere in the library — a runner which neither guards nor awaits lets a sync throw propagate out of `resolve`/`dispose` and drops an async rejection. `OnDisposeModule` hands the runner the actions of **every** instance of the disposed scope as one flat list, so ordering across instances is the runner's too, and no module calls its runner when nothing was collected. `predicate`, `createExecutionContext` and `mapExecutionContext` default on the collector and can be overridden per `getActions` call. `getHooks` itself merges the prototype chain on every call and returns a fresh map; `HookCollector` memoizes it internally (`memoize` in `lib/utils/memoize.ts`, also exported for callers who read hooks outside a collector), so collecting on a hot event reads a class's metadata once per key.
...
`IContainer.getInjector()` hands out the injector shared by the whole scope tree (ADR 0014 reversed ADR 0012's "no accessor" rule). These events are the entire wiring surface — nothing in the library subscribes to them for you. `@hook` has no "no hook means invoke the method" shorthand — always name the hook.

Prefer collecting on resolve — `registered` + `provider.onResolved`, or the `onResolve(...)` pipe — over collecting on construction when documenting or writing hooks: construction is the injector's event, so it misses dependencies the injector never builds (`fromValue` constants, factory registrations) and observes the instance before the provider's `decorate(...)` chain. A resolve hook runs on every dependency leaving a provider, after decoration.
