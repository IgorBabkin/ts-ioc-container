...
- `pnpm exec jest __tests__/container/IocContainer.spec.ts`: run a single test file.
...
## Testing Guidelines
Jest with `ts-jest` is the test runner. Add tests alongside the relevant area of the codebase and prefer behavior-focused specs that cover container resolution, scopes, hooks, and token behavior. Use file names like `FeatureName.spec.ts`. Run `pnpm test` locally before submitting; use `pnpm run test:coverage` for larger changes.
...
Do not edit generated outputs directly. Update source files in `lib/`, and for README changes edit `.readme.hbs.md` and regenerate with `pnpm run generate:docs`.
