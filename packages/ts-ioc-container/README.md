...
- `@inject(args)` — resolves the **whole** args array (note: no parentheses; `args` is the inject function itself), or `[]` when nothing was passed
- Works together with `token.args(...)` to pass typed dependencies through the args context

`argsFn(predicate)` is the general form: it iterates the runtime `args` array and returns the **first argument matching** `predicate(value, index)` — think `args.find(predicate)`. `arg(index)` is just a shortcut for matching by position: `arg(0)` is `argsFn((value, index) => index === 0)`. Every `InjectFn` receives `(scope, options)`, where `options.args` is the runtime args array.
