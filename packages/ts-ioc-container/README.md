...
#### Why it is keyed by the constructor, not by name

A `DependencyKey` is a `string | symbol`, so the obvious key for a class would
be `Target.name`. These providers are kept in a map of their own, keyed by the
**constructor object** instead, and that is deliberate: `Target.name` is not a
unique identifier. Two classes in different modules can share a name, and a
minifier renames both to something short and collision-prone. Keying by name
would also collide with a registration bound to the same string — a
`R.fromClass(Logger)` registers under `'Logger'` — so `resolve(Logger)` could
hand back a completely unrelated provider.

Because the constructor is the key, `resolve(SomeClass)` is immune to this. The
**string-keyed side is not** — see
[Name collisions and symbol tokens](#name-collisions-and-symbol-tokens).
...
The container itself avoids the problem wherever it picks a key on your behalf:
the [`TransientProvider`](#transientprovider) behind `resolve(SomeClass)` is
keyed by the constructor object, not by `SomeClass.name`. Registration keys are
yours to choose, so choose one that cannot collide.
