...
A namespace says **where** a dependency is resolved from: the module path a token was given — `__dirname` in a real module — plus its key. `new SingleToken('ILogger', { namespace: __dirname })` declared in `/app/src/domain/user` has the namespace name `/app/src/domain/user/ILogger`, and `LoggerToken.namespace(__dirname)` points an existing token at the module resolving it.

`namespace(template)` restricts a registration to the module tree its glob template covers, so a dependency registered for the domain layer is invisible to everything else.

- `*` matches exactly one segment, `**` matches any number of them — use `/domain/**` for everything below a directory
- a template is matched against the **end** of the namespace name, so it needs no leading `**` to skip the absolute prefix `__dirname` brings
- a template can select by directory (`/domain/**`), by key (`**/ILogger`), or by both (`/domain/*/ILogger`)
- several templates on one registration are alternatives; a registration with none is reachable from everywhere, as before

> [!IMPORTANT]
> A restricted provider also denies a resolution which names **no** namespace, so `container.resolve('ILogger')` does not reach a namespaced registration. Denial behaves exactly like `scopeAccess` denial: resolution cascades to the parent scope and ends in `DependencyNotFoundError`.

> [!WARNING]
> A scope holds one provider per key, so registering the same key twice in one scope still overwrites — namespace templates decide **which callers reach a provider**, not which of several providers under one key answers.
