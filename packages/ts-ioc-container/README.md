...
- [Registration](#registration) `@register`
  - [Token](#token) `bindTo`
  - [Name collisions and symbol tokens](#name-collisions-and-symbol-tokens) `Symbol`
  - [Scope](#scope) `scope`
- [Module](#module)
- [Hook](#hook) `@hook`
...
- Collision-proof token: `new SingleToken<T>(Symbol('IService'))`
...
- `new Provider((container, options) => container.construct(Logger, options))`
...
> [!NOTE]
> Internal, not public API — described here only because it explains what
> `container.resolve(SomeClass)` does. You never construct or import one.

`resolve` takes a `DependencyKey` or a constructor. A key arrives with a
provider behind it; a constructor arrives with nothing, so the container creates
an ad-hoc provider to adapt the class to the provider interface — a
`TransientProvider` over [`construct`](#container), the raw injector step — and
`resolve` has a single path either way.

Being an adapter rather than a registration, it carries nothing: no decorators,
no cache, no access rules. It hands over the pure injector value, or a lazy
proxy of it when the resolve asked for one (`resolve(SomeClass, { lazy: true })`).
Hence *transient* — a new instance on every resolve. A class that needs a
singleton, pipes, or visibility rules needs a registration, and is then resolved
by its key. The two are independent lookups:
...
What this buys you is reach: the container announces the adapter to
`onProviderRegistered` like any provider, which is what lets
[`@onResolved`](#onresolved) hooks run for a class nobody registered. One is
kept per class per scope and disposed with it, identified by the constructor
rather than by `SomeClass.name` — so a bare-constructor resolve is unaffected by
the name collisions the [string-keyed side](#name-collisions-and-symbol-tokens)
can hit.
...
### Name collisions and symbol tokens

Registration keys share one flat namespace per container, and `R.fromClass(X)`
defaults its key to `X.name` — a plain string. Two classes that produce the same
string therefore land on the same key, and **the later registration silently
overwrites the earlier one**. Nothing throws; you just resolve the wrong class.

```typescript
class Report {}      // registers under 'Report'
class Summary {}     // renamed to 'Report' by a minifier — also under 'Report'

container.resolve('Report'); // whichever registered last
```

Two ways this happens in practice, neither of them exotic:

- **Same-named classes in different modules.** `UserService` in two features,
  `Config` in three.
- **Minification.** Bundlers rename classes, so `X.name` becomes something short
  and collision-prone, and a key that was unique in development is not in
  production. Nothing in the type system notices.

Resolving by constructor is unaffected — that path identifies a class by the
constructor itself, not by its name. Registration keys are yours to choose, so
choose one that cannot collide.

> [!IMPORTANT]
> Prefer a **`Symbol`** over a plain string for a token you control. Every
> `Symbol('IReport')` is a distinct value even when two of them share that
> description, so two features cannot collide on a name — and a minifier cannot
> change what the token is, because the token is the symbol, not the class name.
>
> ```typescript
> export const ReportToken = new SingleToken<IReport>(Symbol('IReport'));
>
> @register(bindTo(ReportToken))
> class Report implements IReport {}
>
> ReportToken.resolve(container); // typed, and collision-proof
> ```
>
> A plain string key is fine for a small app you control end to end. Reach for a
> symbol for anything a library exports, anything shipped through a bundler, or
> any key more than one feature registers against.
