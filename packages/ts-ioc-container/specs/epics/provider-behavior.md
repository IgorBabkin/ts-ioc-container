...
### Story: Resolve a class by its constructor through a provider
...
- `container.resolve(SomeClass)` stands the class up behind a provider, so a
  constructor target and a key target follow the same resolution path.
- That provider hands out the pure injector value, or a lazy proxy of it when
  the resolve asked for one.
